namespace :star_events do
  desc 'Fetch star events using GitHub GraphQL API (batched requests). hours=N or FETCH_HOURS=N to set lookback period (default: 2)'
  task :fetch_graphql, [:hours] => :environment do |_, args|
    token = ENV['GITHUB_TOKEN']
    raise 'GITHUB_TOKEN is required for the GraphQL fetch task' unless token

    hours = (args[:hours] || ENV['FETCH_HOURS'] || 2).to_i
    since = hours.hours.ago

    Rails.logger.info "[star_events:fetch_graphql] start: hours=#{hours}, since=#{since}"
    Rails.logger.info "[star_events:fetch_graphql] #{User.count} users found"
    ...
    User.find_each do |user|
      futures << Concurrent::Future.execute(executor: pool) do
        Rails.logger.info "[star_events:fetch_graphql] fetching followings for @#{user.username}"
        followings = user.followings
        Rails.logger.info "[star_events:fetch_graphql] @#{user.username}: #{followings.size} followings"
    ...
    futures.each do |future|
      future.value
      if future.rejected?
        Rails.logger.error "[star_events:fetch_graphql] failed to fetch followings: #{future.reason.class}: #{future.reason.message}"
      end
    end
